...
Mirrors the structure of `agents/summary_agent/agent.py`:
- builds the chat client via the shared helper,
...
def validate_refine_json(raw_text: str) -> tuple[bool, str]:
    ...
    """
    logger.info("Validating refine JSON | chars=%d", len(raw_text))
    try:
        parsed = RefineOutput.model_validate_json(raw_text)
        logger.info("Refine JSON validated successfully")
        return True, parsed.model_dump_json(indent=2)
    except ValidationError as exc:
        logger.warning("Refine JSON validation failed: %s", exc)
