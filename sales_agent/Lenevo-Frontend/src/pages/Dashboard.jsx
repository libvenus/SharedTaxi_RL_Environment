...
import { fetchQuarterPulse, fetchNotifications, fetchTodos, fetchMeetings } from "../api/client";
...
export default function SalesDashboard() {
  ...
  useEffect(() => {
    fetchQuarterPulse("055DAFE7-9840-451D-8328-5F70A6326C03")
      .then((res) => setQuarterPulse(res))
      .catch((err) => console.error("Failed to fetch quarter pulse:", err));

    fetchNotifications("055DAFE7-9840-451D-8328-5F70A6326C03", 6)
      .then((res) => setNotifications(res?.items || []))
      .catch((err) => console.error("Failed to fetch notifications:", err));

    fetchTodos({ filter_type: "all" })
  ...
  const accountFullText =
    "Proposal revision needed by Jun 17 for pricing and legal validation. Scope now covers 1,400 units with DaaS onboarding in phase one. Customer requested delivery split across four regional distribution hubs. Finance asked for updated TCO assumptions before final approval signoff. Legal team has flagged two clauses in the MSA that require revision before the contract can proceed to signature. Procurement confirmed budget allocation but is awaiting VP sign-off on the revised commercial terms. Delivery timeline has been pushed to Q3 pending customs clearance documentation for the APAC shipment.";
  const accountShortText =
    "Proposal revision needed by Jun 17 for pricing and legal validation...";

  const toggleRow = (key) => {
    setExpandedRows((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  const getAccountText = (isExpanded) => {
    if (isExpanded) return accountFullText;
    const cut = accountFullText.lastIndexOf(" ", 240);
    return accountFullText.slice(0, cut) + "...";
  };
            ...
            <div className="sd-accounts-header">
              <h2 className="sd-accounts-title">
                ...
                <span className="sd-accounts-meta">
                  11 total accounts ·{" "}
                  <span className="sd-accounts-risk">1 at risk</span>
                </span>
              </h2>
            ...
            {[
              {
                ...
                statusClass: "sd-status-at-risk",
                dotClass: "sd-status-dot-red",
                statusLabel: "At Risk",
              },
              {
                key: "techstart",
                name: "TechStart Health",
                statusClass: "sd-status-needs-attention",
                dotClass: "sd-status-dot-amber",
                statusLabel: "Needs Attention",
              },
              {
                key: "modern",
                name: "Modern Logistics",
                statusClass: "sd-status-opportunity",
                dotClass: "sd-status-dot-blue",
                ...
                <div className="sd-account-task-row">
                  <div className="sd-account-task-body">
                    <div className="sd-account-task-title">
                      ThinkPad Fleet Refresh
                    </div>
                    <p
                      className={`sd-account-task-desc${
                        expandedRows[acct.key]
                          ? " sd-account-task-desc-expanded"
                          : ""
                      }`}
                    >
                      {getAccountText(expandedRows[acct.key])}
                    </p>
                  </div>

                  <button
                    type="button"
                    className={`sd-account-chevron-btn${
                      expandedRows[acct.key] ? " is-expanded" : ""
                    }`}
                    onClick={() => toggleRow(acct.key)}
                    aria-label="Toggle account details"
                  >
                    <svg
                      className="sd-account-chevron sd-chevron-svg"
                      viewBox="0 0 16 16"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
