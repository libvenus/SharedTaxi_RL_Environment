...
// }
/* ---------- Meeting Prep -------------------------------------------- */
export function fetchMeetings({ filter = "all_meetings", seller_id } = {}, signal) {
  return request("ai-api/meeting-prep/meetings", {
    signal,
    params: { filter, sellerId: seller_id },
  });
}
...
/* ---------- Todos --------------------------------------------------- */
export function fetchTodos({ filter_type = "all", show_completed = false, seller_id } = {}, signal) {
  return request("ai-api/todos/", {
    signal,
    params: { filter_type, showCompleted: show_completed, sellerId: seller_id },
  });
}
...
export async function fetchTranscriptFromVexa(meeting_id) {
  const res = await fetch(`/ai-api/meeting-prep/${meeting_id}/transcript/fetch`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
  if (!res.ok) {
