...
from app.services.meeting_prep import create_meeting, get_upcoming_meetings, send_bot_to_meeting, _poll_vexa_bot
...
def join_meeting_with_bot(
    meeting_id: str,
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
):
    result = send_bot_to_meeting(db=db, meeting_id=meeting_id)
    bot_id = result.get("vexa_bot_id")
    if bot_id:
        background_tasks.add_task(_poll_vexa_bot, meeting_id, str(bot_id))
    return result


@router.post(
    "/{meeting_id}/transcript/fetch",
    summary="Pull transcript from Vexa and store it in the database",
    responses={
        200: {"description": "Transcript fetched and stored"},
        404: {"description": "Meeting not found or no Vexa bot associated"},
        502: {"description": "Vexa unreachable or returned an error"},
    },
)
def fetch_transcript_from_vexa(meeting_id: str, db: Session = Depends(get_db)):
    from app.services.meeting_prep import fetch_and_store_vexa_transcript
    return fetch_and_store_vexa_transcript(db=db, meeting_id=meeting_id)
